    ...
    public class SorterVision : VisionBase
    {
        ...
        private void DetectGoodPickupTablets(Image<Bgr, Byte> src, CircleF[] tablets)
        {
            ...
            for (int i = 0; i < tablets.Length; i++)
            {
                ...
                int[][] hue = getHighLowHSV(histo, HSVDetectionThershold, HSVdata.Hue);
                int[][] sat = getHighLowHSV(histo, HSVDetectionThershold, HSVdata.Sat);
                int[][] val = getHighLowHSV(histo, HSVDetectionThershold, HSVdata.Val);
