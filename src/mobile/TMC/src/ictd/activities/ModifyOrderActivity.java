...
public class ModifyOrderActivity extends Activity
{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		mOrderOwner.setText(getIntent().getStringExtra(Constants.NAME));
		mOrderId.setText(getIntent().getStringExtra(Constants.ID));
	...
	public void onModifyOrderClicked(View view)
	{
		Order newOrder = new Order(
				Integer.decode(mOrderId.getText().toString()), mOrderOwner
	...
	private void makeModifyService(Order order)
	{
		...
		parcel.putString("orderName", order.getOrderOwner());
	...
	private void handleModifyOrderResult(String response, Context context)
	{
		...
		if ((response).equalsIgnoreCase("\"success\""))
		{
			...
			Toast.makeText(context, Constants.MODIFY_ORDER_FAIL,
					Toast.LENGTH_SHORT).show();
	}
